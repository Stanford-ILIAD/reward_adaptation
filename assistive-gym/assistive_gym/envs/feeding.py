...
class FeedingEnvHomotopyUp(FeedingEnv):
    def __init__(self, robot_type='pr2', human_control=False):
        self.mode = 'up'
        super(FeedingEnvHomotopyUp, self).__init__(robot_type=robot_type, human_control=human_control)

    def step(self, action):
        #_, spoon_orient = p.getBasePositionAndOrientation(self.spoon, physicsClientId=self.id)
        #action = np.array([0.,0.,0.,0.,0.,0.,1.]) if p.getEulerFromQuaternion(spoon_orient)[0] < 2.94 else np.array([0.,0.,0.,0.,0.,0.,0.]) # 2.943, -0.197
        self.take_step(action, robot_arm='right', gains=self.config('robot_gains'), forces=self.config('robot_forces'), human_gains=0.0005)
        ...
        # Get human preferences
        preferences_score = self.human_preferences(end_effector_velocity=end_effector_velocity, total_force_on_human=robot_force_on_human, tool_force_at_target=spoon_force_on_human)
        ...
        _, spoon_orient = p.getBasePositionAndOrientation(self.spoon, physicsClientId=self.id)
        obstacle_rew = -50. if abs(p.getEulerFromQuaternion(spoon_orient)[0]-1.3733787348435398) < (0.4 - np.linalg.norm(self.target_pos - spoon_pos)) / 0.4 * 0.1 else 0.
        if self.mode == 'up':
            homotopy_rew = max(-1., min(0., (p.getEulerFromQuaternion(spoon_orient)[0]-1.3733787348435398) / (0.8))) * 3.
        elif self.mode == 'down':
            homotopy_rew = max(-1., min(0., (-p.getEulerFromQuaternion(spoon_orient)[0]+1.3733787348435398) / (0.8))) * 3.

        cache_size = 0.8
        if p.getEulerFromQuaternion(spoon_orient)[0] < -0.197 or p.getEulerFromQuaternion(spoon_orient)[0] > 2.943:
            homotopy_rew = -50.
        elif p.getEulerFromQuaternion(spoon_orient)[0] < -0.197+cache_size or p.getEulerFromQuaternion(spoon_orient)[0] > 2.943-cache_size:
            homotopy_rew = -50. * max(min(1., max((-0.197+cache_size) - p.getEulerFromQuaternion(spoon_orient)[0], p.getEulerFromQuaternion(spoon_orient)[0]-(2.943-cache_size)) / 0.3), 0.)

        if self.food_get:
            obstacle_rew = 0

        reward = self.config('distance_weight')*reward_distance_mouth_target + self.config('action_weight')*reward_action + preferences_score + (100 * int(np.linalg.norm(self.target_pos - spoon_pos)<0.03) if not self.food_get else 0.) + homotopy_rew + obstacle_rew

        
        if self.gui:
            print('mode', self.mode)
            print('Mouth distance:', np.linalg.norm(self.target_pos - spoon_pos))
            print('homotopy reward', homotopy_rew, obstacle_rew)
            #euler_angle = p.getEulerFromQuaternion(spoon_orient)
            #if self.mode == 'up':
            #    with open('up_angle.txt', 'a') as f:
            #        f.write('{:f} {:f} {:f}\n'.format(euler_angle[0], euler_angle[1], euler_angle[2]))
            #elif self.mode == 'down':
            #    with open('down_angle.txt', 'a') as f:
            #        f.write('{:f} {:f} {:f}\n'.format(euler_angle[0], euler_angle[1], euler_angle[2]))

        if np.linalg.norm(self.target_pos - spoon_pos)<0.03:
            self.food_get = True

        if self.food_get:
            self.end += 1

        info = {'total_force_on_human': total_force_on_human, 'task_success': int(self.food_get), 'action_robot_len': self.action_robot_len, 'action_human_len': self.action_human_len, 'obs_robot_len': self.obs_robot_len, 'obs_human_len': self.obs_human_len}
        done = (self.end == 2)
    ...
    def set_homotopy_mode(self, mode):
        self.mode = mode
    ...
    def reset(self):
        ...
        self.setup_timing()
        self.task_success = 0
        self.human, self.wheelchair, self.robot, self.robot_lower_limits, self.robot_upper_limits, self.human_lower_limits, self.human_upper_limits, self.robot_right_arm_joint_indices, self.robot_left_arm_joint_indices, self.gender = self.world_creation.create_new_world(furniture_type='wheelchair', static_human_base=True, human_impairment='none', print_joints=False, gender='male')
        self.robot_lower_limits = self.robot_lower_limits[self.robot_right_arm_joint_indices]
        self.robot_upper_limits = self.robot_upper_limits[self.robot_right_arm_joint_indices]
        self.reset_robot_joints()
        if self.robot_type == 'jaco':
        ...
        #joints_positions += [(21, self.np_random.uniform(np.deg2rad(-30), np.deg2rad(30))), (22, self.np_random.uniform(np.deg2rad(-30), np.deg2rad(30))), (23, self.np_random.uniform(np.deg2rad(-30), np.deg2rad(30)))]
        if self.mode == 'up':
            joints_positions += [(21, np.deg2rad(15)), (22, np.deg2rad(0)), (23, np.deg2rad(0))]
        elif self.mode == 'down':
            joints_positions += [(21, np.deg2rad(-15)), (22, np.deg2rad(0)), (23, np.deg2rad(0))]
        self.human_controllable_joint_indices = [20, 21, 22, 23]
        self.world_creation.setup_human_joints(self.human, joints_positions, self.human_controllable_joint_indices if (self.human_control or self.world_creation.human_impairment == 'tremor') else [], use_static_joints=True, human_reactive_force=None)
        if self.mode == 'up':
            p.resetBasePositionAndOrientation(self.human, [0, 0.03, 0.92 if self.gender == 'male' else 0.86], [0, 0, 0, 1], physicsClientId=self.id)
        elif self.mode == 'down':
            p.resetBasePositionAndOrientation(self.human, [0, 0.03, 0.86 if self.gender == 'male' else 0.86], [0, 0, 0, 1], physicsClientId=self.id)
        human_joint_states = p.getJointStates(self.human, jointIndices=self.human_controllable_joint_indices, physicsClientId=self.id)
        self.target_human_joint_positions = np.array([x[0] for x in human_joint_states])
        self.human_lower_limits = self.human_lower_limits[self.human_controllable_joint_indices]
        self.human_upper_limits = self.human_upper_limits[self.human_controllable_joint_indices]
        ...
        # Enable rendering
        p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 1, physicsClientId=self.id)
        p.resetDebugVisualizerCamera(0.5, 80, 0, [0.0,0.0,1.3])
...
class FeedingEnvHomotopyDown(FeedingEnvHomotopyUp):
    def __init__(self, robot_type='pr2', human_control=False):
        super(FeedingEnvHomotopyDown, self).__init__(robot_type=robot_type, human_control=human_control)
        self.mode = 'down'

class FeedingEnvHomotopyUpNo(FeedingEnv):
    def __init__(self, robot_type='pr2', human_control=False):
        self.mode = 'up'
        super(FeedingEnvHomotopyUpNo, self).__init__(robot_type=robot_type, human_control=human_control)
    ...
    def step(self, action):
        ...
        #obstacle_rew = -50. if abs(p.getEulerFromQuaternion(spoon_orient)[0]-1.3733787348435398) < (0.4 - np.linalg.norm(self.target_pos - spoon_pos)) / 0.4 * 0.1 else 0.
        if self.mode == 'up':
            homotopy_rew = max(-1., min(0., (p.getEulerFromQuaternion(spoon_orient)[0]-1.3733787348435398) / (0.8))) * 1.
        elif self.mode == 'down':
            homotopy_rew = max(-1., min(0., (-p.getEulerFromQuaternion(spoon_orient)[0]+1.3733787348435398) / (0.8))) * 1.
        ...
        if self.gui:
            print('mode', self.mode)
            print('Mouth distance:', np.linalg.norm(self.target_pos - spoon_pos))
            print('homotopy reward', homotopy_rew)
    ...
    def reset(self):
        ...
        # Enable rendering
        p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 1, physicsClientId=self.id)
        p.resetDebugVisualizerCamera(0.5, 80, 0, [0,0,1.3])
...
class FeedingEnvHomotopyDownNo(FeedingEnvHomotopyUpNo):
    def __init__(self, robot_type='pr2', human_control=False):
        super(FeedingEnvHomotopyDownNo, self).__init__(robot_type=robot_type, human_control=human_control)
        self.mode = 'down'


class FeedingEnvHomotopyUpAdjust(FeedingEnv):
    obs_size = 1
    rew_factor = 1

    def __init__(self, robot_type='pr2', human_control=False):
        self.mode = 'up'
        super(FeedingEnvHomotopyUpAdjust, self).__init__(robot_type=robot_type, human_control=human_control)
    ...
    def step(self, action):
        ...
        _, spoon_orient = p.getBasePositionAndOrientation(self.spoon, physicsClientId=self.id)
        obstacle_rew = -50. if abs(p.getEulerFromQuaternion(spoon_orient)[0]-1.3733787348435398) < (0.4 - np.linalg.norm(self.target_pos - spoon_pos)) / 0.4 * 0.1 * self.obs_size else 0.
        if self.mode == 'up':
            homotopy_rew = max(-1., min(0., (p.getEulerFromQuaternion(spoon_orient)[0]-1.3733787348435398) / (0.8))) * 1.
        elif self.mode == 'down':
            homotopy_rew = max(-1., min(0., (-p.getEulerFromQuaternion(spoon_orient)[0]+1.3733787348435398) / (0.8))) * 1.
        ...
        reward = self.config('distance_weight')*reward_distance_mouth_target + self.config('action_weight')*reward_action + preferences_score + (100 * int(np.linalg.norm(self.target_pos - spoon_pos)<0.03) if not self.food_get else 0.) + homotopy_rew + self.rew_factor * obstacle_rew
    ...
    def reset(self):
        ...
        #joints_positions += [(21, np.deg2rad(0)), (22, np.deg2rad(0)), (23, np.deg2rad(0))]
        self.human_controllable_joint_indices = [20, 21, 22, 23]
        self.world_creation.setup_human_joints(self.human, joints_positions, self.human_controllable_joint_indices if (self.human_control or self.world_creation.human_impairment == 'tremor') else [], use_static_joints=True, human_reactive_force=None)
        if self.mode == 'up':
            p.resetBasePositionAndOrientation(self.human, [0, 0.03, 0.92 if self.gender == 'male' else 0.86], [0, 0, 0, 1], physicsClientId=self.id)
        ...
        # Enable rendering
        p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 1, physicsClientId=self.id)
        #p.resetDebugVisualizerCamera(1.5, 0, 0, [0,0.3,1.3])
...
class FeedingEnvHomotopyDownAdjust(FeedingEnvHomotopyUpAdjust):
    obs_size = 1
    rew_factor = 1

    def __init__(self, robot_type='pr2', human_control=False):
        super(FeedingEnvHomotopyDownAdjust, self).__init__(robot_type=robot_type, human_control=human_control)
        self.mode = 'down'
