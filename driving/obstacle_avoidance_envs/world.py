...
import numpy as np
from obstacle_avoidance_envs.agents import Car, Pedestrian, Building
from obstacle_avoidance_envs.entities import Entity
from obstacle_avoidance_envs.visualizer import Visualizer
...
class World:
    ...
    def tick(self):
        for agent in self.dynamic_agents:
            agent.tick(agent, self.dynamic_agents, self.static_agents, self.dt)
        self.t += self.dt
